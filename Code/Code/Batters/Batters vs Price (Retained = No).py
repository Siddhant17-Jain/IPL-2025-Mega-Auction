...
# Ensure BASRA+ and Winning Bid (cr) are numeric (if they are strings, coerce errors)
df['BASRA+'] = pd.to_numeric(df['BASRA+'], errors='coerce')
df['Winning Bid'] = pd.to_numeric(df['Winning Bid'], errors='coerce')

# Filter the data to include only players where Retained = No
df = df[df['Retained'] == 'No']
...
plt.title('BASRA+ vs. Winning Bid (Non-Retained Players)', fontsize=14)
plt.xlabel('BASRA+ (100 scale)', fontsize=12)
